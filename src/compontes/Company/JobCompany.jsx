...
import { viweAllJobByCompany } from '../../store/action/getData';
import { connect } from 'react-redux';

const JobCompany = ({ dispatch, res }) => {
    ...
    useEffect(() => {
        dispatch(viweAllJobByCompany(id))
    }, []);
    ...
    useEffect(() => {
        const result = data && data.filter(item => {
            return item.position.toLowerCase().match(search.toLowerCase())
        })
        setFilter(result)
    }, [search])
                                            ...
                                            return (
                                                <tr className='capitalize'>
                                                    ...
                                                    <td class="border-t-0 px-6 align-center border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                                        {val.type == 1 ? <button className='bg-[#7A2048] font-bold text-[#fff] px-2 py-1 rounded'>Full Time</button> : <button className='bg-[#1E2761] font-bold text-[#fff] px-2 py-1 rounded'>Part Time</button>}
                                                    </td>
                                                    <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                                        {val.vacancy}
                                                    </td>
...
const mapStateToProps = (state) => ({
    res: state.viweAllJobByCompany
})
export default connect(mapStateToProps)(JobCompany)